    ...
    for v in w1:
        for h in w2:
            ...
            if v[0][1] < h[0][1] < v[1][1] and h[0][0] < v[0][0] < h[1][0]:
                ...
                temp.append(abs(v[0][0]) + abs(h[1][1]))
                debug_wires_crossing.append(temp)
            temp = []
    return
...
def steps(wc):
    ...
    for cw in temp1:
        print(cw)
        if cw[0][1] < 0 and cw[3][1] < 0:
            if cw[0][1] > cw[3][1]:
                temp_steps = abs(cw[0][1]) + abs(cw[3][1]) + abs(cw[2])
            else:
                temp_steps = abs(cw[0][1]) - abs(cw[3][1]) + abs(cw[2])
        elif cw[0][1] > 0 and cw[3][1] > 0:
            if cw[0][1] > cw[3][1]:
                temp_steps = abs(cw[0][1]) - abs(cw[3][1]) + abs(cw[2])
            else:
                temp_steps = abs(cw[0][1]) + abs(cw[3][1]) + abs(cw[2])
            ...
            if cw[0][1] > cw[3][1]:
                ...
                temp_steps = abs(cw[0][1]) + abs(cw[3][1]) + abs(cw[2])
        if cw[0][0] < 0 and cw [3][0] < 0:
            if cw[0][0] > cw[3][0]:
                temp_steps += abs(cw[3][0]) - abs(cw[0][0]) + abs(cw[5])
            else:
                temp_steps += abs(cw[0][0]) - abs(cw[3][0]) + abs(cw[5])
        elif cw[0][0] > 0 and cw [3][0] > 0:
            if cw[0][0] > cw[3][0]:
                temp_steps += abs(cw[0][0]) - abs(cw[3][0]) + abs(cw[5])
            else:
                temp_steps += abs(cw[0][0]) + abs(cw[3][0]) + abs(cw[5])
...
if __name__ == "__main__":
    ...
    steps(debug_wires_crossing)

    print(mdist)
    print('Smallest Manhattan distance: ', min(mdist))
    print('Minimum steps: ', min(csteps))
    ...
    with open('crossing_wires.txt', 'w') as F:
        for w in debug_wires_crossing:
            F.write(str(w))
            F.write('\n')
