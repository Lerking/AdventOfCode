...
def reverse_wirering(w):
    for e in w:
        if e[3] == False:
            w[0][0], w[0][1] = w[0][1], w[0][0]
            del(w[3])
        else:
            del(w[3])
    return
...
if __name__ == "__main__":
    ...
    reverse_wirering(WIRE1_horiz)
    reverse_wirering(WIRE1_vert)
    reverse_wirering(WIRE2_horiz)
    reverse_wirering(WIRE2_vert)
